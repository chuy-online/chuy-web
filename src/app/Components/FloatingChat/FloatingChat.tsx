...
//   const renderMessageWithFormatting = (text: string) => {
//     const parts = text.split(/(\*\*[^*]+\*\*)|(click aquí)/g);
//     return parts.map((part, index) => {
//       if (part?.startsWith("**") && part.endsWith("**")) {
//         return <strong key={index}>{part.slice(2, -2)}</strong>;
//       }
...
//           <a
//             key={index}
//             href="https://www.chuypublicidad.com/cotizar"
//             // target="_blank"
...
const FloatingChat = () => {
  ...
  // Floating text timer effect
  useEffect(() => {
    // Show floating text initially
    ...
    // First interval: show every 2 minutes
    ...
    // After 10 minutes, switch to 5-minute interval
    const switchIntervalTimeout = setTimeout(() => {
      clearInterval(initialInterval);

      // Second interval: show every 5 minutes
    ...
    return () => {
      ...
      clearInterval(initialInterval);
      clearTimeout(switchIntervalTimeout);
    };
  }, []);
  ...
  useEffect(() => {
    ...
    if (
      savedChat &&
      expiration &&
      new Date().getTime() < Number.parseInt(expiration)
    ) {
      setMessages(JSON.parse(savedChat));
    } else {
      localStorage.removeItem(CHAT_STORAGE_KEY);
  ...
  const toggleChat = () => {
    ...
    if (!isOpen) {
      setHasNewMessage(false);
      setIsFirstOpen(false);
      setShowFloatingText(false); // Hide floating text when chat is opened
    }
  };
    ...
    <div>
      {!isOpen && (
        <div className="chat-icon-container">
          {showFloatingText && (
            <div className="floating-text">¡Chatea con nosotros!</div>
          )}
          <div className="chat-icon" onClick={toggleChat}>
            <FiMessageCircle size={35} />
            {(hasNewMessage || isFirstOpen) && (
